	...
	<div class="ckeckout">
		<div class="container">
			...
			<table class="table table-bordered">
				<thead>
					<tr>
						...
						<th>Nama Kost</th>
						<th>Harga</th>
						<th>Transfer Rekening</th>
					</tr>
				</thead>
				<tbody>
					<?php $nomor=1; ?>
					<?php foreach ($_SESSION['cart'] as $id_kost => $jumlah): ?>
			...
			<div class="form-group">
				<input type="text" readonly value="<?php echo $_SESSION["pelanggan"]['nama_cus'] ?>" class="form-control">
				</div>
			</div>
			<div class="col-md-6">
			<div class="form-group">
				<input type="text" readonly value="<?php echo $_SESSION["pelanggan"]['telepon_cus'] ?>" class="form-control">
			</div>
		</div>
		 ...
		 if (isset($_POST["checkout"])) 
		 {
		 	...
		 	$tanggal_book = date("Y-m-d");
		 	$totalbelanja = $totalharga;

		 	//saving into booking table
		 	$koneksi->query("INSERT INTO booking (id_customer,tanggal_booking,total_booking) VALUES ('$id_pelanggan','$tanggal_book','$totalbelanja') ");

		 	//get current id_booking
		 	$id_currentbooking = $koneksi->insert_id;

		 	foreach ($_SESSION["cart"] as $id_kost => $jumlah) 
		 	{
		 		$koneksi->query("INSERT INTO booking_kost (id_booking,id_kost,jumlah) VALUES ('$id_currentbooking','$id_kost','$jumlah')");
		 	}


		 	//redirect ke halaman nota
		 	echo "<script>alert('Sukses');</script>";
		 	echo "<script>location='nota.php?id=$id_currentbooking';</script>";
		 }
